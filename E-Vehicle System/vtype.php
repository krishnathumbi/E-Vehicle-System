...
if(isset($_POST['craet']))
{
	...
	$vtype=$_POST['vtype'];
	$res = mysql_query("select * from vehicletype where VehicleName = '$vtype'");
	if($res)
	{
		$dbh=new dbi();
	$sql=mysql_query("insert into vehicletype (VehicleName) values ('".$vtype."')");
	if($sql)
	{
		$msg="vehicle added";
	...
	{
		$msg="vehicle Type alresdy inserted";
	}
