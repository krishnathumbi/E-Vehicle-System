...
$('.tam').val(otalamt);
 ...
 $('.nocars').blur(function(){
			                                     $(".nocars").html("Checker availability...");
			                                     var usr = $(".carchecker").val();
		                                          $.ajax({
		                                          url:"http:/localhost/carrent/test1.php?action=carchecker&nocars="+usr,
												  type:'POST',
												  data: "nocars="+usr,
												  success: function(msg) {if(msg==1){$(".nocars");
												   $("").val('');
												  } else{$(".carchecker").html("");} }
