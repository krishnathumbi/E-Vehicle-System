...
var cartypenew=$(this).parent().prev('.myclassouter').children('.carchecker').attr('value');

	 var noscar=$(this).val();
	   if ($('.noscar').text() != content)
		{
			// $.post("test1.php?nocars="+noscar);
			 $(".checkingava").load("test1.php?nocars="+noscar+"&cartype="+cartypenew);
            /* $(".myclassrouter").load("test1.php?cartype="+cartype);  */ 	
		}
...
<input type="checkbox" id="cbx" name="checkboxx"value="<?php echo $result->cartype;?>" class="carchecker">
<?php  echo $result->cartype;?></div>
<div class="carcheckerclass" style="display:none"><input type="text"  name="typeno" class="nocars myClass caas<?php echo $result->cartype;?>" style="width:20px; float:left;" value="0" id="cs<?php echo $result->cartype;?>">
